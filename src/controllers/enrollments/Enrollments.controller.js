...
exports.createEnrollment = async (req, res) => {
  ...
  try {
    ...
    const errorMap = {
      ...
      "Student already enrolled in a section for this school year": 409,
    };
...
exports.updateEnrollment = async (req, res) => {
  const { id } = req.params;
  const { student_id, grade_level_id, school_year_id, section_id, curriculum_id, enrollment_date, status } = req.body;
  const userId = req.user?.user_id;
  const enrollmentId = parseInt(id);
  ...
  if (!student_id || !Number.isInteger(student_id) || student_id <= 0) {
    return res.status(400).json({
      success: false,
      error: 'Student ID is required and must be a positive integer',
      timestamp: new Date().toISOString()
    });
  }
  ...
  try {
    const updatedEnrollment = await updateEnrollmentById(enrollmentId, {
      student_id,
      grade_level_id,
      school_year_id,
      section_id,
    ...
    if (error.message.includes('not found')) {
      return res.status(404).json({ success: false, error: error.message, timestamp: new Date().toISOString() });
    }
...
exports.deleteEnrollment = async (req, res) => {
  ...
  try {
    const result = await deleteEnrollmentById(enrollmentId, userId);
