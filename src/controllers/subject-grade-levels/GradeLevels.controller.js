...
exports.createSubjectGradeLevel = async (req, res) => {
  ...
  if (
    !grade_level_id ||
    !Number.isInteger(grade_level_id) ||
    grade_level_id <= 0
  ) {
    return res
      .status(400)
  ...
  if (
    units !== undefined &&
    (typeof units !== "number" || units < 0 || units > 99.9)
  ) {
    return res
      .status(400)
      .json({
  ...
  try {
    ...
    return res.status(201).json({
      success: true,
      message:
        "Subject assigned to grade level in active curriculum successfully",
    ...
    if (error.message.includes("already exists")) {
      return res
        ...
        .json({
          success: false,
          error:
            "Subject is already assigned to this grade level for the active curriculum",
          timestamp: new Date().toISOString(),
        });
    }
...
exports.deleteSubjectGradeLevel = async (req, res) => {
  ...
  try {
    const result = await deleteSubjectGradeLevelById(
      subjectIdInt,
      gradeLevelIdInt,
      userId
    );
    ...
    if (error.message.includes("Cannot delete subject grade level")) {
      return res.status(409).json({
        success: false,
        error:
          "Cannot delete subject grade level assignment as it is being used in curriculum or enrollments",
        timestamp: new Date().toISOString(),
      });
    }
  ...
  for (let i = 0; i < assignments.length; i++) {
    ...
    if (!subject_id || !Number.isInteger(subject_id) || subject_id <= 0) {
      return res.status(400).json({
        ...
        error: `Assignment ${
          i + 1
        }: Subject ID is required and must be a positive integer`,
        timestamp: new Date().toISOString(),
      });
    }
    ...
    if (
      ...
      return res.status(400).json({
        ...
        error: `Assignment ${
          i + 1
        }: Grade level ID is required and must be a positive integer`,
        timestamp: new Date().toISOString(),
      });
    }
